...
class Buffer {
	...
	public function new(w,h,up, fl_transp:Bool, col:Int, ?useScale2x=false, ?pixelRatio=1.0) {
		width = w;
		height = h;
		upscale = up;
	...
	public function changeScale(s:Int) {
		throw "not supported yet (todo)";
		
		if( fl_scale2x )
			throw "not supported with scale2x yet";
	...
	public static function makeMosaic2(wid:Float, ?col=0xffffff, ?alpha=1.0) {
		...
		var w = Color.addAlphaF(col, alpha);
	...
	public inline function update() {
		if( !destroyed() ) {
			...
			#if flash11_3
			if( drawQuality!=null )
				renderBD.drawWithQuality(container, drawQuality);
			else
