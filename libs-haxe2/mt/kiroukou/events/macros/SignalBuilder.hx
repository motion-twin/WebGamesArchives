package mt.kiroukou.events.macros;
...
class SignalBuilder
{
	...
	@:macro public static function build():Array<Field> {
		...
		var BASE_SIGNAL = "mt.kiroukou.events.Signal";
		
        var pos = haxe.macro.Context.currentPos();
		
        var tstring = macro : String;
		var tvoid = macro : Void;
		var thash = macro : Hash<Array<Dynamic>>;
		var tbond = macro : mt.kiroukou.events.Signal;
		...
		for( field in sigFields )
		{
			...
			var prefix = "";
			var id = 0;
			while( !mt.kiroukou.tools.StringTools.isAlpha(name.charAt(id)) ) id++;
			if( id > 0 )
			...
			case FFun( f ):
				...
				{ // Bind function
					...
					if( as3Signal ) {
						...
						var bond : mt.kiroukou.events.Signal = cast { autoDispose : false, listener : listener };
						...
						//trace(tink.macro.tools.ExprTools.toString(fbody));
					} else {
						for( arg in f.args )
							argTypes.push( arg.type );
			...
			fields.push( { name : "signaler_listenerHash", access:[APrivate], kind:FVar( thash ), pos : pos } );
			{
				var fbody = macro {
					if( signaler_listenerHash == null )
						signaler_listenerHash = new Hash<Array<Dynamic>>();
