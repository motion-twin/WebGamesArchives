	...
	public function new(m : flash.MovieClip, t : AnimType, f : Transition, params : Dynamic) {
		...
		switch(type) {
			...
			case Color(c, s) :
				//nothing to do
			case Rotation(s)  : 
				initRot = (m._rotation * 3.14 / 180) % 6.28 ;
				toRot = params.r ;
	...
	public function update() {
		...
		switch (type) {
			...
			case Scale : 
				...
				mc._xscale = sx * (1 - delta) + ex * delta ;
				mc._yscale = sy * (1 - delta) + ey * delta ;
			...
			case Rotation(s) : 
				mc._rotation = (initRot + delta * toRot * s) * 180 / 3.14 ;
