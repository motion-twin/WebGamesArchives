...
class Dynamit extends StageObject {
	...
	public function new(v : Int, ?dm : mt.DepthManager, ?depth : Int = 2, ?noOmc = false, ?withBmp : flash.display.BitmapData, ?sc : Int) {
		super() ;
		value = v ;
		id = Const.getArt(_Dynamit(v)) ;
		vertical = v > 0 ;
		horizontal = v != 1 ;
		checkHelp() ;
		...
		omc = new ObjectMc(_Dynamit(v), pdm,depth, null, null, null, withBmp, sc) ;
	}
