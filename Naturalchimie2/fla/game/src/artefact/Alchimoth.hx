...
class Alchimoth extends StageObject {
	...
	public function new(?dm : mt.DepthManager, ?depth : Int, ?noOmc = false, ?withBmp : flash.display.BitmapData, ?sc : Int) {
		super() ;
		id = Const.getArt(_Alchimoth) ;
		checkHelp() ;
		...
		omc = new ObjectMc(
					_Alchimoth,
					pdm,
					if (depth == null) 2 else depth, null, null, null, withBmp, sc) ;
	}
