...
class Group  {
	...
	static var ONLY_ONE_PARASIT = [_PolarBomb] ; 
	...
	public function new(?e : _ArtefactId, ?hide : Bool = false, ?notSame = false) {
		r = 0 ;
		depthr = 0 ;
		targetR = null ;
		isMoving = false ;
	...
	public function initObjects(?e : _ArtefactId, ?hide : Bool, notSame : Bool) {
		objects = new Array() ;
		
		if (e == null)
			e = Game.me.mode.getNext(hide) ;
		...
		switch(e) {
			case _Elts(n, p) :
				...
				for (i in 0...n) {
					...
					var o = null ;
					if (r < 0) { //parasit 
						if ( countParasit > 0 && Lambda.exists(ONLY_ONE_PARASIT, function(x : _ArtefactId) { return Type.enumEq(x, parasit) ; }))
							o = cast new Element(Game.me.mode.getRandomElement(false, 0), dm, depth) ;
						else
							o = cast StageObject.get(parasit, dm, depth) ;
					} else 
						o = cast new Element(r, dm, depth) ;
	...
	public function update() {		
		...
		var ds = 0.0 ;
		//if(((Const.BOT_MODE && (untyped Game.me.playInfos).x < col && !isMoving) || (!noMove && Key.isDown(Key.LEFT))) && bigx > 0 ) {
		if(((Const.BOT_MODE && (untyped Game.me.playInfos).x < col && !isMoving) || (!noMove && Game.me.pLeft)) && bigx > 0 ) {
			col = bigx - 1;
			ds = -s ;
			isMoving = true;
		//} else if(((Const.BOT_MODE && (untyped Game.me.playInfos).x > col && !isMoving) || (!noMove && Key.isDown(Key.RIGHT))) && calcx < maxx ) {
		} else if(((Const.BOT_MODE && (untyped Game.me.playInfos).x > col && !isMoving) || (!noMove && Game.me.pRight)) && calcx < maxx ) {
			col = calcx + 1;
			ds = s;
			isMoving = true;
		} else { //recal
	...
	public function toStage(?f) {
		if (!Const.BOT_MODE)
			...
			mc.setMask((cast Game.me).groupMask) ;
		...
		for (o in objects)
			o.onStage() ;
		Game.me.stage.mcEffect.toHide = false ;
	...
	public function startRotate() {
		if (isRotating || objects.length <= 1)
			return ;
		Game.me.sound.playRotate() ;
