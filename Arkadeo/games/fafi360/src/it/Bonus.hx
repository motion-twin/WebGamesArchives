...
class Bonus extends GenericItem {
	...
	public function new(big:Bool) {
		...
		if( !big ) {
			...
			value = switch(frame) {
				case 1 : api.AKApi.const(50);
				case 2 : api.AKApi.const(250);
				default : api.AKApi.const(0);
			}
		}
		else {
			// gros bonus
			...
			value = switch( frame ) {
				...
				case 3 : api.AKApi.const(500);
				case 4 : api.AKApi.const(750);
				case 5 : api.AKApi.const(1000);
				case 6 : api.AKApi.const(1500);
				case 7 : api.AKApi.const(2000);
				default : api.AKApi.const(500);
			}
			mc.filters = [
	...
	public override function pickUp() {
		...
		fx.popScore(xx,yy, value.get(), color, big);
		fx.pick(xx,yy, color);
		destroy();
	}
