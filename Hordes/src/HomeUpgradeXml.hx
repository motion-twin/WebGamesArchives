...
class HomeUpgradeXml {
	...
	public static function getLimit(upgrade:HUpgrade, dbLevel:Int) {
		var lvl = getCurrentLevel(upgrade,dbLevel);
		var limit = lvl.limit;
		if (upgrade.key=="cook" && App.user.map.hasCityBuilding("centralCafet"))
			limit+=3;
		if (upgrade.key=="labo" && App.user.map.hasCityBuilding("centralLab"))
			limit+=5;
		return limit;
	}
