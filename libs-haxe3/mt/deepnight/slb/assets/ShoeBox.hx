...
#error "ShoeBox support is deprecated, it was not updated recently"
...
class ShoeBox {
	public static macro function importXml(xmlUrl:String) {
		...
		blockContent.push(
			macro var _lib = mt.deepnight.slb.assets.ShoeBox.parseXml($rscGetter, $newSourceExpr)
		);
		...
		if( animFile!=null ) {
			...
			for( a in xml.nodes.a ) {
				...
				var frames = try{ BLib.parseAnimDefinition(a.innerHTML, a.has.timing ? Std.parseInt(a.att.timing):null); } catch(e:Dynamic) { AssetTools.error(animUrl+" parse error for group "+group.toUpperCase()+": "+e, p); null; };
	...
	#if !macro
	public static function downloadXml(xmlUrl:String, imgUrl:String, onComplete:BLib->Void) {
		var xml : String = null;
