...
class Color {
	...
	public static inline function offsetColor(c:Col, delta:Int) : Col {
		return {
			r	: Std.int( MLib.fmax(0, MLib.fmin(255,c.r + delta)) ),
			g	: Std.int( MLib.fmax(0, MLib.fmin(255,c.g + delta)) ),
			b	: Std.int( MLib.fmax(0, MLib.fmin(255,c.b + delta)) ),
		}
	}
	...
	//public static inline function mix(?fromRgb:Col, ?fromInt:Int, ?toRgb:Col, ?toInt:Int, ratio:Float) {
		//if( fromInt!=null )
			//fromRgb = intToRgb(fromInt);
//
		//if( toInt!=null )
			//toRgb = intToRgb(toInt);
//
		//return rgbToInt( interpolate(fromRgb, toRgb, ratio) );
	//}
	...
	public static inline function colorizeBatchElement(e:h2d.SpriteBatch.BatchElement, c:UInt, ?ratio=1.0) {
		var c = Color.intToRgb( mt.deepnight.Color.interpolateInt(0xFFFFFF, c, ratio) );
		e.r = c.r/255;
		e.g = c.g/255;
		e.b = c.b/255;
	}
	...
	public static inline function makePaletteCustom(colors:Array<{ratio:Float, col:Int}>) : PalInt { // du sombre au clair
		...
		var pal : PalInt = [];
		var idx = 0;
		var colors = colors.map( function(c) return {idx:MLib.round(c.ratio*256), col:c.col} );
	...
	public static function makeWhiteGradient(wid,hei, light:UInt, dark:UInt, ?white:UInt) : h3d.mat.Texture {
		...
		var pal = mt.deepnight.Color.makeNicePalette(light, dark, white, true);
		for(x in 0...wid)
			for(y in 0...hei)
				p.setPixel(x, y, pal[MLib.round(pal.length*x/wid)]);
	...
	public static function makeLinearGradient(wid, hei, light:UInt, dark:UInt) : h3d.mat.Texture {
		var p = hxd.Pixels.alloc(wid,hei, ARGB);
		var pal = mt.deepnight.Color.makePaletteLinear([light,dark], 256);
		for(x in 0...wid)
			for(y in 0...hei)
				p.setPixel(x, y, pal[MLib.round(pal.length*x/wid)]);
