class FPerlin {
	...
	inline function gradient3D( x : Float, y : Float, z : Float, ix : Int, iy : Int, iz : Int, seed : Int ) {
		var index = seed * 1013 + ix * 1619 + iy * 31337 + iz * 6971;
	...
	inline function gradient( x : Float, y : Float, ix : Int, iy : Int, seed : Int ) {
		var index = seed * 1013 + ix * 1619 + iy * 31337;
		index = ((index ^ (index >>> 8)) & 0xFF) << 5;
		var gx = double(index);
		var gy = double(index + 8);
	...
	static inline var BIG = 1073741824.0;
	...
	public function add3D( scale : Float, power : Float = 1.0 ) {
		if( scale >= 1 ) {
			seed++;
			return;
		}
		var out = outOffset, x = x * scale, y = y * scale, z = z * scale;
	...
	public function adjust( scale : Float, power : Float ) {
		if( scale >= 1 ) {
			seed++;
			return;
		}
		...
		for( py in 0...height ) {
			...
			for( px in 0...width ) {
				...
				var gd = gradient(x, y, ix + 1, iy + 1, seed);
				var v = linear(linear(ga, gb, xs), linear(gc, gd, xs), ys);
				setDouble(out, double(out) * (v * power + 1));
				x += scale;
				out += 8;
			}
			y += scale;
